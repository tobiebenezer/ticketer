...
class _TicketsHistoryScreenState extends State<TicketsHistoryScreen> {
  ...
  Future<void> _loadFiles() async {
    ...
    final appEntries = await appDir.list().toList();
    allPdfs.addAll(
      appEntries
          .whereType<File>()
          .where((f) => f.path.toLowerCase().endsWith('.pdf'))
          .where((f) => f.path.toLowerCase().contains('ticket')),
    );
    ...
    if (Platform.isAndroid) {
      ...
      if (await downloadsDir.exists()) {
        try {
          final downloadEntries = await downloadsDir.list().toList();
          allPdfs.addAll(
            downloadEntries
                .whereType<File>()
                .where((f) => f.path.toLowerCase().endsWith('.pdf'))
          ...
          print('Could not access Downloads folder: $e');
        }
      }
    }
